...
/* helpers */
static tga_image::tga_result tga_read_rle(tga_image *dest, std::istream& fp);
static tga_image::tga_result tga_write_row_RLE(std::ostream& fp, const tga_image *src, const uint8_t *row);
...
uint8_t tga_image::tga_get_attribute_bits(const tga_image *tga)
{
    return tga->image_descriptor & ImageDescriptorBits::TGA_ATTRIB_BITS;
}

int tga_image::tga_is_right_to_left(const tga_image *tga)
{
    return (tga->image_descriptor & ImageDescriptorBits::TGA_R_TO_L_BIT) != 0;
}

int tga_image::tga_is_top_to_bottom(const tga_image *tga)
{
    return (tga->image_descriptor & ImageDescriptorBits::TGA_T_TO_B_BIT) != 0;
}

int tga_image::tga_is_colormapped(const tga_image *tga)
{
    return (
        tga->image_type == tga_image::TGA_IMAGE_TYPE_COLORMAP ||
        tga->image_type == tga_image::TGA_IMAGE_TYPE_COLORMAP_RLE
    );
}

int tga_image::tga_is_rle(const tga_image *tga)
{
    return (
        tga->image_type == tga_image::TGA_IMAGE_TYPE_COLORMAP_RLE  ||
        tga->image_type == tga_image::TGA_IMAGE_TYPE_BGR_RLE       ||
        tga->image_type == tga_image::TGA_IMAGE_TYPE_MONO_RLE
    );
}

int tga_image::tga_is_mono(const tga_image *tga)
{
    return (
        tga->image_type == tga_image::TGA_IMAGE_TYPE_MONO      ||
        tga->image_type == tga_image::TGA_IMAGE_TYPE_MONO_RLE
    );
}
 ...
 */
const char *tga_image::tga_error(const tga_result errcode)
{
    switch (errcode)
    {
    case TGA_NOERR:
 ...
 *          valid.
 */
tga_image::tga_result tga_image::tga_read(tga_image *dest, std::istream& fp)
{
    #define BARF(errcode) \
        { return errcode; }

    #define READ(destptr, size) \
        fp.read(reinterpret_cast<char*>(destptr), size);\
        if ( ( (fp.rdstate() & std::ifstream::failbit ) != 0 ) || ( (fp.rdstate() & std::ifstream::eofbit ) != 0 ) )\
        {\
            BARF(TGAERR_EOF)\
        }

    #define READ16(dest) READ(dest, 2)
...
static tga_image::tga_result tga_read_rle(tga_image *dest, std::istream& fp)
{
    ...
    while ((p_loaded < p_expected) && ( (fp.rdstate() & std::ifstream::eofbit ) == 0 ))
    {
        uint8_t b;
        READ(&b, 1);
        if (b & RLE_BIT)
        {
            ...
            {
                p_loaded++;
                if (p_loaded > p_expected) return tga_image::TGAERR_RLE;
                memcpy(pos, tmp, bpp);
                pos += bpp;
            }
        }
        else /* RAW packet */
        {
            ...
            count = (b & ~RLE_BIT) + 1;
            if (p_loaded + count > p_expected) return tga_image::TGAERR_RLE;
